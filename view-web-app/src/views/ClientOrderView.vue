...
import {getOrderByUserId} from "@/modules/orderAPI";
...
async function loadAllOrders() {
  ...
  if (userId) {
    const ordersList = await getOrderByUserId(userId);
    const loadedOrders = await loadOrdersDetails(ordersList);
    orders.value = loadedOrders;
    isLoading.value = false;
  }
}
watch(() => store.state.user?.id, async (userId: string | undefined) => {
  loadAllOrders();
});
...
<template>
  <section class="orders-section">
    <div class="orders-header">
      <h2>Affichage de vos commandes :</h2>
